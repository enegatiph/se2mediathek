...
public class VormerkMedienauflisterWerkzeug extends BeobachtbaresSubWerkzeug
{
    ...
    private void setzeAnzuzeigendeMedien()
    {
        ...
        Queue<Kunde> vorgemerkteKunden = null;
        for (Medium medium : medienListe)
        {
            // DONE für Aufgabenblatt 6: Die VormerkMedienFormatierer müssen
            // noch mit einem möglichen Entleiher und möglichen Vormerkern
            // ausgestattet werden.
            vorgemerkteKunden = _verleihService.getVormerkkarteFuer(medium).getVormerker();
            Kunde entleiher = (_verleihService.getVerleihkarteFuer(medium) != null) ? _verleihService.getVerleihkarteFuer(medium).getEntleiher() : null;
            Kunde vormerker1 = (vorgemerkteKunden.size() > 0) ? vorgemerkteKunden.remove() : null;
            Kunde vormerker2 = (vorgemerkteKunden.size() > 0) ? vorgemerkteKunden.remove() : null;
            Kunde vormerker3 = (vorgemerkteKunden.size() > 0) ? vorgemerkteKunden.remove() : null;
